...
def standardize_dataframe(df: pd.DataFrame, file_path: Path, split: str, timestamp_override: str = "") -> pd.DataFrame:
    ...
    if ts_col is None:
        ...
        if missing_ts.any():
            df.loc[missing_ts, "timestamp"] = np.arange(missing_ts.sum(), dtype=int)
    ...
    df["weak_label"] = (split == "anomaly_file").astype(int) if isinstance(split, pd.Series) else int(split == "anomaly_file")
